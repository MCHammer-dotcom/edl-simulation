# -------------------- edl_experiment_extended_v1_5.py --------------------
"""
...
# 🔧 Add project root to sys.path so 'src' and 'config' can be found
...
type_means = (final.groupby(["group", "actor_type"])["total_value"]
              .mean().unstack().round(2) if "actor_type" in final else None)
...
if vol_track is not None:
    print(f"\nVolatility mean={vol_track.mean():.3f}  ρ_t range=({rho_track.min():.2f}–{rho_track.max():.2f})")
