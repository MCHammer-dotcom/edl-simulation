# ---------------------- edl_boosted_experiment.py ----------------------
"""
Boosted-Productivity Experiment
===============================
...
4. Boost-Both           – orchestrant boosts both terms

All other parameters are held constant (γ=0.08, λ=0.05).

Outputs
-------
boosted_value_trajectories.png   – total-value lines
boosted_delta_plot.png           – Δ(Treatment−Control) lines
...
import ecosim                      # requires v1.2 with boost logic
# ---------------------------------------------------------------------
OUTDIR = Path(".")
COMMON = dict(
    n_actors=50,
...
CONDITIONS = {
    ...
    "Boost-Both":       dict(orchestrant_boost_operant=True,
                             orchestrant_boost_operand=True)
}
# ---------------------------------------------------------------------
def run_condition(name: str, flags: dict) -> pd.DataFrame:
    """Run ecosim and tag DataFrame with group label."""
...
def plot_trajectories(stats_df, groups):
    ...
    plt.title("Total Value Trajectories")
...
def plot_delta(stats_df, groups):
    ...
    plt.title("Value Gain Over Baseline"); plt.grid(alpha=0.4); plt.legend()
...
def plot_decomposition(stats_df, groups):
    ...
    fig.suptitle("Component Decomposition"); fig.tight_layout()
...
def main():
    ...
    # plots
    group_order = list(CONDITIONS.keys())
    plot_trajectories(stats_df, group_order)
    plot_delta(stats_df, group_order)
    plot_decomposition(stats_df, group_order)
    ...
    print("\nNote: Productivity boosts model the contextual enablement "
          "that orchestrant resources provide, in line with EDL theory. "
          "Curves plateau as the system approaches externality-bounded equilibrium.")
